...
static float32x4_t copy_lane(float32x4_t vdst, int dlane, float32x4_t vsrc,
                             int slane) {
#if GLM_ARCH & GLM_ARCH_ARMV8_BIT
  switch (dlane) {
  case 0:
  ...
  default: assert(!"Unreachable code executed!");
  }
  switch (dlane) {
...
static float32x4_t madd_lane(float32x4_t acc, float32x4_t v, float32x4_t vlane,
                             int lane) {
#if GLM_ARCH & GLM_ARCH_ARMV8_BIT
#ifdef GLM_CONFIG_FORCE_FMA
#define FMADD_LANE(acc, x, y, L)                                               \
  do {                                                                         \
    asm volatile("fmla %0.4s, %1.4s, %2.4s"                                    \
                 : "+w"(acc)                                                   \
                 : "w"(x), "w"(dup_lane(y, L)));                               \
  } while (0)
#else
#define FMADD_LANE(acc, x, y, L)                                               \
  do {                                                                         \
    acc = vmlaq_laneq_f32(acc, x, y, L);                                       \
  } while (0)
#endif

  switch (lane) {
  case 0: FMADD_LANE(acc, v, vlane, 0); return acc;
  case 1: FMADD_LANE(acc, v, vlane, 1); return acc;
  case 2: FMADD_LANE(acc, v, vlane, 2); return acc;
  case 3: FMADD_LANE(acc, v, vlane, 3); return acc;
  default: assert(!"Unreachable code executed!");
  }
  assert(!"Unreachable code executed!");
  return vdupq_n_f32(0.0f);
